...
IN_DIM_MERMAS = SILVER / "dim_mermas_ajuste_fecha_post_destino.parquet"
IN_DIM_VAR = SILVER / "dim_variedad_canon.parquet"
IN_B2_SHARE_REAL = SILVER / "dim_b2_share_real_fecha_post_variedad_grado_destino.parquet"


def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser("build_views_ml1_ml2_real_compare")
    ap.add_argument("--pred-ml2", default=None, help="Legacy ML2 path (treated as global input).")
    ap.add_argument("--pred-ml2-global", default=None, help="Path pred_ml2_multitask_nn_global_*.parquet")
...
def _latest_pred_ml2() -> Path:
    files = sorted(GOLD_ML2.glob("pred_ml2_multitask_nn_global_*.parquet"))
    if files:
        return files[-1]
...
def _run_id_from_path(path: Path) -> str:
    m = re.search(r"pred_ml2_multitask_nn_(?:global_|puro_|operativo_)?(.+)\.parquet$", path.name)
...
def _load_real_post_alloc(base: pd.DataFrame) -> pd.DataFrame:
    post = base.loc[
        base["stage"].eq("POST"),
        ["row_id", "fecha_evento", "fecha_post", "grado", "destino", "share_block_post", "variedad_canon"],
    ].copy()
    ...
    # peso_post_g from this fact is hydration output (pre mermas/ajuste final).
    fr["kg_hidr_total"] = _num(fr.get("peso_post_g"), default=0.0) / 1000.0

    key = ["fecha_evento", "fecha_post", "grado_int", "destino"]
    agg = (
        fr[key + ["dh_dias", "hidr_pct", "tallos_total", "kg_verde_total", "kg_hidr_total"]]
        .dropna(subset=key)
        .groupby(key, dropna=False, as_index=False)
        .agg(
            ...
            kg_hidr_real_total=("kg_hidr_total", "sum"),
        )
    )

    m = post.merge(agg, on=key, how="left")
    # Real mermas/ajuste by fecha_post + destino.
    if IN_DIM_MERMAS.exists():
        ma = read_parquet(IN_DIM_MERMAS).copy()
        ma.columns = [str(c).strip() for c in ma.columns]
        if "fecha_post" in ma.columns and "destino" in ma.columns:
            ma["fecha_post"] = _to_date(ma["fecha_post"])
            ma["destino"] = _canon_str(ma["destino"])
            if "factor_desp" in ma.columns:
                ma["factor_desp_real"] = _num(ma["factor_desp"], default=np.nan)
            elif "desp_pct" in ma.columns:
                ma["factor_desp_real"] = 1.0 - _num(ma["desp_pct"], default=np.nan)
            else:
                ma["factor_desp_real"] = np.nan
            if "factor_ajuste" in ma.columns:
                ma["factor_ajuste_real"] = _num(ma["factor_ajuste"], default=np.nan)
            elif "ajuste" in ma.columns:
                aj = _num(ma["ajuste"], default=np.nan)
                ma["factor_ajuste_real"] = np.where(aj > 0.0, 1.0 / aj, np.nan)
            else:
                ma["factor_ajuste_real"] = np.nan

            ma2 = (
                ma.groupby(["fecha_post", "destino"], dropna=False, as_index=False)
                .agg(
                    factor_desp_real=("factor_desp_real", "median"),
                    factor_ajuste_real=("factor_ajuste_real", "median"),
                )
            )
            m = m.merge(ma2, on=["fecha_post", "destino"], how="left")
        else:
            m["factor_desp_real"] = np.nan
            m["factor_ajuste_real"] = np.nan
    else:
        m["factor_desp_real"] = np.nan
        m["factor_ajuste_real"] = np.nan

    # Real destination distribution from BALANZA 2 in silver.
    if IN_B2_SHARE_REAL.exists():
        b2s = read_parquet(IN_B2_SHARE_REAL).copy()
        b2s.columns = [str(c).strip() for c in b2s.columns]
        need_b2s = {"fecha_post", "variedad_canon", "grado_int", "destino", "share_dest_real_b2"}
        if need_b2s.issubset(set(b2s.columns)):
            b2s["fecha_post"] = _to_date(b2s["fecha_post"])
            b2s["variedad_canon"] = _canon_str(b2s["variedad_canon"])
            b2s["grado_int"] = _to_int(b2s["grado_int"])
            b2s["destino"] = _canon_str(b2s["destino"])
            for c in [
                "share_dest_real_b2",
                "b2_peso_dest_real_kg",
                "share_dest_real_b2_gv",
                "share_dest_real_b2_v",
                "share_dest_real_b2_d",
                "fallback_level_b2",
            ]:
                if c not in b2s.columns:
                    b2s[c] = np.nan
            b2_take = b2s[
                [
                    "fecha_post",
            ...
            ].drop_duplicates(subset=["fecha_post", "variedad_canon", "grado_int", "destino"])
            m = m.merge(
                b2_take,
                on=["fecha_post", "variedad_canon", "grado_int", "destino"],
                how="left",
            )
        else:
            m["share_dest_real_b2"] = np.nan
            m["b2_peso_dest_real_kg"] = np.nan
    else:
        m["share_dest_real_b2"] = np.nan
        m["b2_peso_dest_real_kg"] = np.nan
    ...
    m["kg_hidr_real"] = _num(m["kg_hidr_real_total"], default=np.nan) * m["share_alloc"]
    f_desp = _num(m.get("factor_desp_real"), default=np.nan)
    f_aj = _num(m.get("factor_ajuste_real"), default=np.nan)
    has_ma = f_desp.notna() | f_aj.notna()
    kg_post_from_fac = (
        _num(m["kg_verde_real"], default=np.nan)
        * _num(m["factor_hidr_real"], default=np.nan)
        * f_desp.fillna(1.0)
        * f_aj.fillna(1.0)
    )
    m["kg_post_real"] = pd.to_numeric(
        np.where(has_ma, kg_post_from_fac, _num(m["kg_hidr_real"], default=np.nan)),
        errors="coerce",
    )
...
def _balance_ml2_to_tallos_proy(v: pd.DataFrame, preserve_real: bool = False) -> None:
    m_hg = v["stage"].eq("HARVEST_GRADE")
    m_post = v["stage"].eq("POST")
    if not m_hg.any():
        return

    m_hg_real = pd.Series(False, index=v.index)
    if preserve_real and "ml2_anchor_real" in v.columns:
        m_hg_real = m_hg & pd.to_numeric(v["ml2_anchor_real"], errors="coerce").fillna(0.0).gt(0.0)
    m_hg_scale = m_hg & ~m_hg_real
    ...
    if preserve_real:
        cyc_real = (
            v.loc[m_hg_real]
            .groupby("ciclo_id", dropna=False, as_index=False)
            .agg(tallos_real_fixed=("tallos_grado_dia", "sum"))
        )
        cyc_pred = (
            v.loc[m_hg_scale]
            .groupby("ciclo_id", dropna=False, as_index=False)
            .agg(tallos_pred_scalable=("tallos_grado_dia", "sum"))
        )
        ...
        # Dynamic target update: if observed real already exceeds original target,
        # keep projected non-real remainder instead of collapsing it to zero.
        cyc["tallos_target_eff"] = np.maximum(
            pd.to_numeric(cyc["tallos_target"], errors="coerce").fillna(0.0),
    ...
    v.loc[m_hg_scale, "tallos_grado_dia"] = pd.to_numeric(v.loc[m_hg_scale, "tallos_grado_dia"], errors="coerce") * sc.loc[m_hg_scale]
    v.loc[m_hg_scale, "tallos_dia"] = pd.to_numeric(v.loc[m_hg_scale, "tallos_dia"], errors="coerce") * sc.loc[m_hg_scale]
    ...
    if "kg_verde" in v.columns:
        v.loc[m_hg_scale, "kg_verde"] = pd.to_numeric(v.loc[m_hg_scale, "kg_verde"], errors="coerce") * sc.loc[m_hg_scale]
        v.loc[m_hg_scale, "gramos_verde"] = pd.to_numeric(v.loc[m_hg_scale, "gramos_verde"], errors="coerce") * sc.loc[m_hg_scale]
...
def _enforce_post_green_from_tallos_peso(v: pd.DataFrame) -> None:
    m_post = v["stage"].eq("POST")
    if not bool(m_post.any()):
        return

    tallos = pd.to_numeric(v.get("tallos_post"), errors="coerce")
    peso_est_g = pd.to_numeric(v.get("peso_tallo_estimado_g"), errors="coerce")
    kg_calc = (tallos * peso_est_g / 1000.0).clip(lower=0.0)
    m_calc = m_post & kg_calc.notna()
    if not bool(m_calc.any()):
        return

    v.loc[m_calc, "kg_verde"] = kg_calc.loc[m_calc]
    v.loc[m_calc, "gramos_verde"] = kg_calc.loc[m_calc] * 1000.0
    # 1 caja verde = 10 kg
    v.loc[m_calc, "cajas_verde"] = kg_calc.loc[m_calc] / 10.0


def _enforce_post_vs_hg_grade_balance(v: pd.DataFrame) -> None:
    """Force POST destino split to match HG grade stems on each event day key."""
    if "stage" not in v.columns:
        return
    m_hg = v["stage"].eq("HARVEST_GRADE")
    m_post = v["stage"].eq("POST")
    if not bool(m_hg.any() and m_post.any()):
        return

    key = [c for c in ["ciclo_id", "fecha_evento", "bloque_base", "variedad_canon", "grado"] if c in v.columns]
    if len(key) < 2:
        return

    hg = (
        v.loc[m_hg, key + ["tallos_grado_dia"]]
        .groupby(key, dropna=False, as_index=False)
        .agg(tallos_hg=("tallos_grado_dia", "sum"))
    )
    if hg.empty:
        return
    ...
    p = v.loc[m_post].copy()
    p["__idx"] = p.index
    p = p.merge(hg, on=key, how="left")
    m_has_hg = pd.to_numeric(p.get("tallos_hg"), errors="coerce").notna()
    if not bool(m_has_hg.any()):
        return
    p = p.loc[m_has_hg].copy()

    tallos_post_cur = pd.to_numeric(p.get("tallos_post"), errors="coerce")
    den = tallos_post_cur.groupby([p[c] for c in key], dropna=False).transform("sum")
    cnt = tallos_post_cur.groupby([p[c] for c in key], dropna=False).transform("size").clip(lower=1)
    mix = pd.Series(
        np.where(den > 0.0, tallos_post_cur / den, 1.0 / cnt.astype(float)),
        index=p.index,
        dtype="float64",
    ).fillna(0.0)
    tallos_hg = pd.to_numeric(p.get("tallos_hg"), errors="coerce").fillna(0.0)
    tallos_post_new = (tallos_hg * mix).clip(lower=0.0)

    idx = p["__idx"].astype(int)
    v.loc[idx, "tallos_post"] = tallos_post_new.to_numpy(dtype="float64")

    # Keep downstream masses/boxes consistent with corrected stems.
    peso_real = pd.to_numeric(v.get("peso_tallo_real_g"), errors="coerce")
    peso_est = pd.to_numeric(v.get("peso_tallo_estimado_g"), errors="coerce")
    peso_use = peso_real.where(peso_real.notna(), peso_est)
    tallos_post_all = pd.to_numeric(v.get("tallos_post"), errors="coerce")
    kg_green = (tallos_post_all * peso_use / 1000.0).clip(lower=0.0)
    m_post_valid = m_post & kg_green.notna()
    if bool(m_post_valid.any()):
        v.loc[m_post_valid, "kg_verde"] = kg_green.loc[m_post_valid]
        v.loc[m_post_valid, "gramos_verde"] = kg_green.loc[m_post_valid] * 1000.0
        v.loc[m_post_valid, "cajas_verde"] = kg_green.loc[m_post_valid] / 10.0
    ...
    m_post_chain = m_post & pd.to_numeric(v.get("kg_verde"), errors="coerce").notna() & prod.notna()
    if bool(m_post_chain.any()):
        v.loc[m_post_chain, "kg_post"] = pd.to_numeric(v.loc[m_post_chain, "kg_verde"], errors="coerce") * prod.loc[m_post_chain]
        v.loc[m_post_chain, "cajas_post"] = pd.to_numeric(v.loc[m_post_chain, "kg_post"], errors="coerce") / 10.0
        v.loc[m_post_chain, "aprovechamiento"] = _safe_div(v.loc[m_post_chain, "kg_post"], v.loc[m_post_chain, "kg_verde"], default=np.nan)


def _enforce_hg_day_consistency(v: pd.DataFrame) -> None:
    # Ensure day stems are always the sum of grade stems on the same day key.
    if v.empty or "stage" not in v.columns:
        return
    if "tallos_grado_dia" not in v.columns or "tallos_dia" not in v.columns:
        return

    m_hg = v["stage"].eq("HARVEST_GRADE")
    if not bool(m_hg.any()):
        return

    day = pd.to_numeric(v.get("day_in_harvest"), errors="coerce").round()
    fev = pd.to_datetime(v.get("fecha_evento"), errors="coerce").dt.normalize()
    day_key = day.where(day.notna(), fev)

    h = pd.DataFrame(
        {
            "ciclo_id": v.loc[m_hg, "ciclo_id"].astype("string"),
            "__day_key": day_key.loc[m_hg],
        }
    )
    for c in ["bloque_base", "variedad_canon"]:
        if c in v.columns:
            h[c] = v.loc[m_hg, c]
    h["tallos_grado_dia"] = pd.to_numeric(v.loc[m_hg, "tallos_grado_dia"], errors="coerce").fillna(0.0)

    grp_cols = [c for c in ["ciclo_id", "__day_key", "bloque_base", "variedad_canon"] if c in h.columns]
    if len(grp_cols) < 2:
        return

    hg_day = (
        h.groupby(grp_cols, dropna=False, as_index=False)
        .agg(tallos_dia_fix=("tallos_grado_dia", "sum"))
    )
    map_idx = pd.MultiIndex.from_frame(hg_day[grp_cols])
    day_map = pd.Series(hg_day["tallos_dia_fix"].to_numpy(dtype=np.float64), index=map_idx)

    idx_hg = pd.DataFrame({"ciclo_id": v.loc[m_hg, "ciclo_id"].astype("string"), "__day_key": day_key.loc[m_hg]})
    for c in ["bloque_base", "variedad_canon"]:
        if c in v.columns:
            idx_hg[c] = v.loc[m_hg, c]
    idx_hg_mi = pd.MultiIndex.from_frame(idx_hg[grp_cols])
    v.loc[m_hg, "tallos_dia"] = idx_hg_mi.map(day_map).to_numpy(dtype=np.float64)

    m_post = v["stage"].eq("POST")
    if bool(m_post.any()):
        idx_p = pd.DataFrame({"ciclo_id": v.loc[m_post, "ciclo_id"].astype("string"), "__day_key": day_key.loc[m_post]})
        for c in ["bloque_base", "variedad_canon"]:
            if c in v.columns:
                idx_p[c] = v.loc[m_post, c]
        idx_p_mi = pd.MultiIndex.from_frame(idx_p[grp_cols])
        mapped = idx_p_mi.map(day_map)
        old = pd.to_numeric(v.loc[m_post, "tallos_dia"], errors="coerce")
        v.loc[m_post, "tallos_dia"] = pd.Series(mapped, index=v.loc[m_post].index).where(
            pd.Series(mapped, index=v.loc[m_post].index).notna(),
            old,
...
def _apply_stage_masks(v: pd.DataFrame) -> None:
    ...
    # Keep upstream curve/share/peso factors on POST rows for end-to-end traceability.
    post_na = []
    for c in post_na:
        if c in v.columns:
...
def _dedup_post_business_key(v: pd.DataFrame, prefer_real: bool) -> None:
    if v.empty or "stage" not in v.columns:
        return
    m_post = v["stage"].eq("POST")
    if not bool(m_post.any()):
        return
    key = [c for c in ["ciclo_id", "fecha_evento", "bloque_base", "variedad_canon", "grado", "destino"] if c in v.columns]
    if len(key) < 2:
        return
    p = v.loc[m_post].copy()
    score = pd.Series(0.0, index=p.index, dtype="float64")
    if prefer_real:
        if "ml2_anchor_real" in p.columns:
            score = score + pd.to_numeric(p["ml2_anchor_real"], errors="coerce").fillna(0.0)
        if "tallos_post_real" in p.columns:
            score = score + pd.to_numeric(p["tallos_post_real"], errors="coerce").notna().astype(float)
    score = score + pd.to_numeric(p.get("tallos_post"), errors="coerce").fillna(0.0)
    p["__score"] = score
    sort_cols = ["__score"]
    asc = [False]
    if "row_id" in p.columns:
        sort_cols.append("row_id")
        asc.append(False)
    p = p.sort_values(sort_cols, ascending=asc, kind="mergesort")
    p = p.drop_duplicates(subset=key, keep="first").drop(columns=["__score"])
    out = pd.concat([v.loc[~m_post], p], ignore_index=True)
    order_cols = [c for c in ["ciclo_id", "fecha_evento", "stage", "bloque_base", "variedad_canon", "grado", "destino"] if c in out.columns]
    if order_cols:
...
def _build_view_ml1(base: pd.DataFrame, run_id: str) -> pd.DataFrame:
    ...
    # ML1 timeline should follow ML1 projected harvest rows (pure model output),
    # not an inferred VEG start that can be inconsistent after chain rebuild.
    hg_obs = base.loc[base["stage"].eq("HARVEST_GRADE"), ["ciclo_id", "fecha_evento"]].copy()
    if not hg_obs.empty:
        hg_obs["fecha_evento"] = _to_date(hg_obs["fecha_evento"])
        hg_obs = (
            hg_obs.dropna(subset=["ciclo_id", "fecha_evento"])
            .groupby("ciclo_id", dropna=False, as_index=False)
            .agg(harvest_start_obs=("fecha_evento", "min"), harvest_end_obs=("fecha_evento", "max"))
        )
        hg_obs["n_harvest_days_obs"] = (
            pd.to_datetime(hg_obs["harvest_end_obs"], errors="coerce")
            - pd.to_datetime(hg_obs["harvest_start_obs"], errors="coerce")
        ).dt.days + 1
        v = v.merge(hg_obs, on="ciclo_id", how="left")
        m_obs = pd.to_datetime(v.get("harvest_start_obs"), errors="coerce").notna()
        if m_obs.any():
            v.loc[m_obs, "harvest_start"] = _to_date(v.loc[m_obs, "harvest_start_obs"])
            n_pred = _num(v.loc[m_obs, "n_harvest_days"], default=np.nan)
            n_obs = _num(v.loc[m_obs, "n_harvest_days_obs"], default=np.nan)
            n_use = n_pred.where(n_pred.notna() & (n_pred >= n_obs), n_obs)
            n_use = n_use.clip(lower=1.0)
            v.loc[m_obs, "n_harvest_days"] = n_use
            v.loc[m_obs, "harvest_end"] = (
                pd.to_datetime(v.loc[m_obs, "harvest_start"], errors="coerce")
                + pd.to_timedelta(np.rint(n_use).astype(int) - 1, unit="D")
            ).dt.normalize()
            v.loc[m_obs, "d_start"] = (
                pd.to_datetime(v.loc[m_obs, "harvest_start"], errors="coerce")
                - pd.to_datetime(v.loc[m_obs, "fecha_sp"], errors="coerce")
            ).dt.days
        v = v.drop(columns=[c for c in ["harvest_start_obs", "harvest_end_obs", "n_harvest_days_obs"] if c in v.columns])
    ...
    day_calc = pd.to_numeric(d_calc.loc[m_chain], errors="coerce")
    v.loc[m_chain, "day_in_harvest"] = day_calc.where(day_calc >= 1.0, np.nan)
    ...
    v["peso_tallo_baseline_g"] = _num(base.get("peso_tallo_baseline_g"), default=np.nan)
    v["peso_tallo_estimado_g"] = v["peso_tallo_baseline_g"] * v["factor_peso_tallo"]
    ...
    _enforce_post_green_from_tallos_peso(v)
...
def _build_view_ml2(base: pd.DataFrame, run_id: str, source: str) -> pd.DataFrame:
    ...
    # Only operativo should anchor timeline to real start.
    if source == "ML2_OPERATIVO":
        ...
        # ML2_global / ML2_puro must follow the ML2 projected event axis directly.
        date_axis = _to_date(base.get("fecha_evento_ml2")).fillna(_to_date(base.get("fecha_evento")))
        hg_obs = base.loc[base["stage"].eq("HARVEST_GRADE"), ["ciclo_id"]].copy()
        hg_obs["fev"] = date_axis.loc[hg_obs.index]
        if not hg_obs.empty:
            hg_obs = (
                hg_obs.dropna(subset=["ciclo_id", "fev"])
                .groupby("ciclo_id", dropna=False, as_index=False)
                .agg(harvest_start_obs=("fev", "min"), harvest_end_obs=("fev", "max"))
            )
            hg_obs["n_harvest_days_obs"] = (
                pd.to_datetime(hg_obs["harvest_end_obs"], errors="coerce")
                - pd.to_datetime(hg_obs["harvest_start_obs"], errors="coerce")
            ...
            if m_obs.any():
                ...
                n_pred = _num(v.loc[m_obs, "n_harvest_days"], default=np.nan)
                n_obs = _num(v.loc[m_obs, "n_harvest_days_obs"], default=np.nan)
                n_use = n_pred.where(n_pred.notna() & (n_pred >= n_obs), n_obs).clip(lower=1.0)
                v.loc[m_obs, "n_harvest_days"] = n_use
                v.loc[m_obs, "harvest_end"] = (
                    pd.to_datetime(v.loc[m_obs, "harvest_start"], errors="coerce")
                    + pd.to_timedelta(np.rint(n_use).astype(int) - 1, unit="D")
        ...
        # Align ML2 timelines to the observed-real harvest start available from VEG labels
        # (target_d_start), while keeping ML2 projected factors/curve behavior.
        veg_real = base.loc[base["stage"].eq("VEG"), ["ciclo_id", "fecha_evento", "target_d_start"]].copy()
        if not veg_real.empty:
            veg_real = veg_real.sort_values("fecha_evento", kind="mergesort").drop_duplicates(subset=["ciclo_id"], keep="last")
            veg_real["fecha_sp_real"] = _to_date(veg_real["fecha_evento"])
            d_real = _num(veg_real.get("target_d_start"), default=np.nan)
            m_d_real = pd.to_numeric(d_real, errors="coerce").notna()
            if m_d_real.any():
                veg_real.loc[m_d_real, "harvest_start_real"] = (
                    pd.to_datetime(veg_real.loc[m_d_real, "fecha_sp_real"], errors="coerce")
                    + pd.to_timedelta(np.rint(pd.to_numeric(d_real.loc[m_d_real], errors="coerce")).astype(int), unit="D")
                ).dt.normalize()
                hs_map = pd.Series(
                    pd.to_datetime(veg_real["harvest_start_real"], errors="coerce").dt.normalize().to_numpy(),
                    index=veg_real["ciclo_id"].astype("string"),
                )
                cid = v["ciclo_id"].astype("string")
                hs_real = pd.to_datetime(cid.map(hs_map), errors="coerce").dt.normalize()
                m_hs_real = hs_real.notna()
                if m_hs_real.any():
                    v.loc[m_hs_real, "harvest_start"] = hs_real.loc[m_hs_real]
                    nd = pd.to_numeric(v.loc[m_hs_real, "n_harvest_days"], errors="coerce").fillna(1.0).clip(lower=1.0)
                    v.loc[m_hs_real, "harvest_end"] = (
                        pd.to_datetime(v.loc[m_hs_real, "harvest_start"], errors="coerce")
                        + pd.to_timedelta(np.rint(nd).astype(int) - 1, unit="D")
                    ).dt.normalize()
                    v.loc[m_hs_real, "d_start"] = (
                        pd.to_datetime(v.loc[m_hs_real, "harvest_start"], errors="coerce")
                        - pd.to_datetime(v.loc[m_hs_real, "fecha_sp"], errors="coerce")
                    ).dt.days

    v["fecha_post"] = _to_date(base.get("fecha_post_ml2")).fillna(_to_date(base["fecha_post"]))
    if "ml2_anchor_real" in base.columns:
        v["ml2_anchor_real"] = pd.to_numeric(base.get("ml2_anchor_real"), errors="coerce").fillna(0.0).gt(0.0)
    else:
        v["ml2_anchor_real"] = False
    ...
    v["peso_tallo_baseline_g"] = _num(base.get("peso_tallo_baseline_g"), default=np.nan)
    v["peso_tallo_estimado_g"] = v["peso_tallo_baseline_g"] * v["factor_peso_tallo"]
    v["peso_tallo_real_g"] = _num(base["peso_tallo_real_g"], default=np.nan) if "peso_tallo_real_g" in base.columns else np.nan
    ...
    # Operativo: when real is available, replace field-stage values (share/peso/tallos/green mass)
    # with observed values, then keep projection only forward.
    if source == "ML2_OPERATIVO":
        hgr, _, _ = _load_real_harvest_join(base)
        if not hgr.empty:
            hr = hgr[
                [
                    "row_id",
                    ...
                    "tallos_grado_real",
                    "kg_verde_grado_real",
                    "factor_tallos_dia_real",
                    "share_grado_real",
                    "factor_peso_tallo_real",
                    "peso_tallo_real_g",
                ]
            ].copy()
            v = v.merge(hr, on="row_id", how="left", suffixes=("", "_hr"))
            m_hg = v["stage"].eq("HARVEST_GRADE")
            m_real_hg = m_hg & pd.to_numeric(v.get("ml2_anchor_real"), errors="coerce").fillna(0.0).gt(0.0)
            if m_real_hg.any():
                v.loc[m_real_hg, "tallos_dia"] = _num(v.loc[m_real_hg, "tallos_dia_real"], default=np.nan).astype("float32")
                v.loc[m_real_hg, "tallos_grado_dia"] = _num(v.loc[m_real_hg, "tallos_grado_real"], default=0.0).astype("float32")
                v.loc[m_real_hg, "kg_verde"] = _num(v.loc[m_real_hg, "kg_verde_grado_real"], default=0.0).astype("float32")
                v.loc[m_real_hg, "gramos_verde"] = v.loc[m_real_hg, "kg_verde"] * 1000.0
                v.loc[m_real_hg, "factor_tallos_dia"] = _num(v.loc[m_real_hg, "factor_tallos_dia_real"], default=np.nan).astype("float32")
                v.loc[m_real_hg, "share_grado"] = _num(v.loc[m_real_hg, "share_grado_real"], default=0.0).astype("float32")
                v.loc[m_real_hg, "factor_peso_tallo"] = _num(v.loc[m_real_hg, "factor_peso_tallo_real"], default=np.nan).astype("float32")
                v.loc[m_real_hg, "peso_tallo_real_g"] = _num(v.loc[m_real_hg, "peso_tallo_real_g_hr"], default=np.nan).astype("float32")
                v.loc[m_real_hg, "peso_tallo_estimado_g"] = _num(v.loc[m_real_hg, "peso_tallo_real_g"], default=np.nan).astype("float32")
            v = v.drop(columns=[c for c in ["peso_tallo_real_g_hr"] if c in v.columns])
        ...
        m_post = v["stage"].eq("POST")
        if m_post.any():
            # Fallback "real-like" post allocation from real HG stems using destino mix.
            key_cols = [c for c in ["ciclo_id", "fecha_evento", "bloque_base", "variedad_canon", "grado"] if c in v.columns]
            mix_num = pd.to_numeric(v.get("tallos_post"), errors="coerce")
            den_mix = mix_num.groupby([v[c] for c in key_cols], dropna=False).transform("sum")
            cnt_mix = mix_num.groupby([v[c] for c in key_cols], dropna=False).transform("size").clip(lower=1)
            ...
            # When real destination split exists in BALANZA 2, replace projected mix
            # by observed share (fecha_post + variedad + grado + destino).
            dest = _canon_str(v.get("destino", pd.Series(["UNKNOWN"] * len(v), index=v.index)))
            share_b2_real = pd.to_numeric(v.get("share_dest_real_b2"), errors="coerce")
            m_b2_real = m_post & dest.isin(["BLANCO", "TINTURADO", "ARCOIRIS"]) & share_b2_real.notna()
            if bool(m_b2_real.any()):
                grp_keys = [v[c] for c in key_cols]
                b2_any = pd.Series(m_b2_real, index=v.index).groupby(grp_keys, dropna=False).transform("any")
                mix_ratio = mix_ratio.where(~b2_any, 0.0)
                mix_ratio = pd.Series(
                    np.where(m_b2_real, share_b2_real, mix_ratio),
                    index=v.index,
                    dtype="float64",
                )
            ...
            tallos_grade_real = pd.to_numeric(v.get("tallos_grado_dia"), errors="coerce")
            tallos_post_calc = (tallos_grade_real * mix_ratio).clip(lower=0.0)
            peso_real = pd.to_numeric(v.get("peso_tallo_real_g"), errors="coerce")
            peso_est = pd.to_numeric(v.get("peso_tallo_estimado_g"), errors="coerce")
            peso_use = peso_real.where(peso_real.notna(), peso_est)
            kg_verde_calc = (tallos_post_calc * peso_use / 1000.0).clip(lower=0.0)
            fac_post = (
                pd.to_numeric(v.get("factor_hidr"), errors="coerce")
                * pd.to_numeric(v.get("factor_desp"), errors="coerce")
                * pd.to_numeric(v.get("factor_ajuste"), errors="coerce")
            )
            kg_post_calc = (kg_verde_calc * fac_post).clip(lower=0.0)

            # Keep real post fact when available. If real is partial by destino, allocate
            # residual stems over missing destinos so the group total stays coherent with HG real.
            grp_keys = [v[c] for c in key_cols]
            tp_real_src = _num(v.get("tallos_post_real"), default=np.nan)
            # If BALANZA 2 real split exists (all destinos), override destination split
            # from HG real stems using those observed shares.
            share_b2_real = pd.to_numeric(v.get("share_dest_real_b2"), errors="coerce")
            m_b2_real = (
                m_post
                & _canon_str(v.get("destino", pd.Series(["UNKNOWN"] * len(v), index=v.index))).isin(["BLANCO", "TINTURADO", "ARCOIRIS"])
                & share_b2_real.notna()
            )
            if bool(m_b2_real.any()):
                b2_any = pd.Series(m_b2_real, index=v.index).groupby(grp_keys, dropna=False).transform("any")
                w_b2 = pd.Series(np.where(m_b2_real, share_b2_real, 0.0), index=v.index, dtype="float64")
                ...
                tp_b2 = (pd.to_numeric(tallos_grade_real, errors="coerce").fillna(0.0) * share_b2_norm).where(b2_any, np.nan)
                tp_b2 = tp_b2.where(
                    _canon_str(v.get("destino", pd.Series(["UNKNOWN"] * len(v), index=v.index))).isin(["BLANCO", "TINTURADO", "ARCOIRIS"]),
                    np.where(b2_any, 0.0, np.nan),
                )
                tp_real_src = tp_real_src.where(~b2_any, tp_b2)

            real_sum = tp_real_src.fillna(0.0).groupby(grp_keys, dropna=False).transform("sum")
            total_target = pd.to_numeric(tallos_grade_real, errors="coerce").fillna(0.0)
            residual = (total_target - real_sum).clip(lower=0.0)
            ...
            # Enforce grade-level mass balance in operativo:
            # sum(tallos_post_real by destino) must match real HG stems for the same
            # (ciclo, fecha_evento, bloque, variedad, grado). If HG real is 0, post real is 0.
            ...
            scale_to_target = pd.Series(
                np.where(grp_sum_adj > 0.0, total_target / grp_sum_adj, np.nan),
                index=v.index,
                dtype="float64",
            )
            ...
            m_need_fill = tp_real_adj.isna() & (total_target > 0.0)
            if m_need_fill.any():
                tp_real_adj.loc[m_need_fill] = (total_target * mix_share_all).loc[m_need_fill]
            tp_real_adj = pd.Series(
                np.where(total_target <= 0.0, 0.0, np.clip(pd.to_numeric(tp_real_adj, errors="coerce"), 0.0, np.inf)),
                index=v.index,
                dtype="float64",
            )

            kg_verde_real_src = _num(v.get("kg_verde_real"), default=np.nan)
            kg_post_real_src = _num(v.get("kg_post_real"), default=np.nan)
            # Green mass in operativo must come from field-real stems and stem weight.
            ...
            fh_real = _num(v.get("factor_hidr_real"), default=np.nan)
            fd_real = _num(v.get("factor_desp_real"), default=np.nan)
            fa_real = _num(v.get("factor_ajuste_real"), default=np.nan)
            has_real_chain = fd_real.notna() | fa_real.notna()
            kg_post_from_real_chain = (kg_verde_adj * fh_real.fillna(_num(v.get("factor_hidr"), default=np.nan)) * fd_real.fillna(1.0) * fa_real.fillna(1.0)).clip(lower=0.0)
            kg_post_adj = pd.Series(
                np.where(
                    has_real_chain,
                    kg_post_from_real_chain,
                    kg_post_real_src.where(kg_post_real_src.notna(), (kg_verde_adj * fac_post).clip(lower=0.0)),
                ),
                index=v.index,
                dtype="float64",
            )
            # If real stems for the row are zero after reconciliation, downstream masses must be zero.
            m_zero_stems = pd.to_numeric(tp_real_adj, errors="coerce").fillna(0.0).le(0.0)
            if m_zero_stems.any():
                kg_verde_adj = kg_verde_adj.where(~m_zero_stems, 0.0)
                kg_post_adj = kg_post_adj.where(~m_zero_stems, 0.0)

            v["tallos_post_real"] = tp_real_adj
            v["kg_verde_real"] = kg_verde_adj
            v["kg_post_real"] = kg_post_adj
            v["factor_hidr_real"] = _num(v.get("factor_hidr_real"), default=np.nan).fillna(_num(v.get("factor_hidr"), default=np.nan))
            v["dh_dias_real"] = _num(v.get("dh_dias_real"), default=np.nan).fillna(_num(v.get("dh_dias"), default=np.nan))
            v["aprovechamiento_real"] = _safe_div(v["kg_post_real"], v["kg_verde_real"], default=np.nan)

            m_post_use = m_post & pd.to_numeric(v.get("tallos_post_real"), errors="coerce").notna()
            if m_post_use.any():
                v.loc[m_post_use, "dh_dias"] = _num(v.loc[m_post_use, "dh_dias_real"], default=np.nan).astype("float32")
                v.loc[m_post_use, "factor_hidr"] = _num(v.loc[m_post_use, "factor_hidr_real"], default=np.nan).astype("float32")
                # Use real post factors when available by fecha_post+destino.
                fd_real = _num(v.loc[m_post_use, "factor_desp_real"], default=np.nan)
                fa_real = _num(v.loc[m_post_use, "factor_ajuste_real"], default=np.nan)
                fd_cur = _num(v.loc[m_post_use, "factor_desp"], default=np.nan)
                fa_cur = _num(v.loc[m_post_use, "factor_ajuste"], default=np.nan)
                v.loc[m_post_use, "factor_desp"] = fd_real.where(fd_real.notna(), fd_cur).astype("float32")
                v.loc[m_post_use, "factor_ajuste"] = fa_real.where(fa_real.notna(), fa_cur).astype("float32")
                v.loc[m_post_use, "tallos_post"] = _num(v.loc[m_post_use, "tallos_post_real"], default=np.nan).astype("float32")
                v.loc[m_post_use, "kg_verde"] = _num(v.loc[m_post_use, "kg_verde_real"], default=np.nan).astype("float32")
                v.loc[m_post_use, "gramos_verde"] = v.loc[m_post_use, "kg_verde"] * 1000.0
                v.loc[m_post_use, "kg_post"] = _num(v.loc[m_post_use, "kg_post_real"], default=np.nan).astype("float32")
                v.loc[m_post_use, "cajas_verde"] = _num(v.loc[m_post_use, "kg_verde"], default=np.nan) / 10.0
                v.loc[m_post_use, "cajas_post"] = _num(v.loc[m_post_use, "kg_post"], default=np.nan) / 10.0
                v.loc[m_post_use, "aprovechamiento"] = _num(v.loc[m_post_use, "aprovechamiento_real"], default=np.nan).astype("float32")

    # For operativo we rely on ML2 output rows as-is to avoid creating synthetic
    # null rows in future days. Global/puro keep horizon expansion for comparability.
    if source != "ML2_OPERATIVO":
        _expand_missing_horizon_rows(v)
    ...
    _dedup_post_business_key(v, prefer_real=source == "ML2_OPERATIVO")
    # After mass-balance scaling, keep exact post-real facts when available.
    if source == "ML2_OPERATIVO":
        m_post = v["stage"].eq("POST")
        m_post_fact = m_post & pd.to_numeric(v.get("kg_post_real"), errors="coerce").notna()
        if m_post_fact.any():
            v.loc[m_post_fact, "kg_post"] = _num(v.loc[m_post_fact, "kg_post_real"], default=np.nan)
            m_t = m_post_fact & pd.to_numeric(v.get("tallos_post_real"), errors="coerce").notna()
            if m_t.any():
                v.loc[m_t, "tallos_post"] = _num(v.loc[m_t, "tallos_post_real"], default=np.nan)
            m_g = m_post_fact & pd.to_numeric(v.get("kg_verde_real"), errors="coerce").notna()
            if m_g.any():
                v.loc[m_g, "kg_verde"] = _num(v.loc[m_g, "kg_verde_real"], default=np.nan)
                v.loc[m_g, "gramos_verde"] = v.loc[m_g, "kg_verde"] * 1000.0
            v.loc[m_post_fact, "aprovechamiento"] = _safe_div(v.loc[m_post_fact, "kg_post"], v.loc[m_post_fact, "kg_verde"], default=np.nan)
        # In operativo, boxes are derived from the final masses (kg/10), not from projected box seeds.
        m_post_box = m_post & pd.to_numeric(v.get("kg_verde"), errors="coerce").notna()
        if m_post_box.any():
            v.loc[m_post_box, "cajas_verde"] = _num(v.loc[m_post_box, "kg_verde"], default=np.nan) / 10.0
        m_post_box2 = m_post & pd.to_numeric(v.get("kg_post"), errors="coerce").notna()
        if m_post_box2.any():
            v.loc[m_post_box2, "cajas_post"] = _num(v.loc[m_post_box2, "kg_post"], default=np.nan) / 10.0
    _enforce_hg_day_consistency(v)
    if source == "ML2_OPERATIVO":
...
def _build_view_real(base: pd.DataFrame, run_id: str) -> pd.DataFrame:
    ...
    if not hgr.empty:
        ...
        v = v.merge(hr, on="row_id", how="left", suffixes=("", "_hr"))
        for c in ["peso_tallo_baseline_g", "peso_tallo_real_g"]:
            c_hr = f"{c}_hr"
            if c_hr in v.columns:
                v[c] = _num(v.get(c_hr), default=np.nan).fillna(_num(v.get(c), default=np.nan))
                v = v.drop(columns=[c_hr])
        ...
        v["peso_tallo_baseline_g"] = _num(v.get("peso_tallo_baseline_g"), default=np.nan)
        v["peso_tallo_real_g"] = _num(v.get("peso_tallo_real_g"), default=np.nan)
        v["peso_tallo_estimado_g"] = v["peso_tallo_real_g"]
...
def main() -> None:
    args = _parse_args()
    pred_global_path = (
        Path(args.pred_ml2_global)
        if args.pred_ml2_global
        else (Path(args.pred_ml2) if args.pred_ml2 else _latest_pred_ml2())
    )
    if not pred_global_path.exists():
        raise FileNotFoundError(f"ML2 global prediction file not found: {pred_global_path}")

    run_id = _run_id_from_path(pred_global_path)
    pred_oper_path = Path(args.pred_ml2_operativo) if args.pred_ml2_operativo else (
        pred_global_path.parent / f"pred_ml2_multitask_nn_operativo_{run_id}.parquet"
    )
    if not pred_oper_path.exists():
        pred_oper_path = pred_global_path
    pred_pure_path = Path(args.pred_ml2_puro) if args.pred_ml2_puro else (
        pred_global_path.parent / f"pred_ml2_multitask_nn_puro_{run_id}.parquet"
    )
    if not pred_pure_path.exists():
        pred_pure_path = pred_global_path

    pred_global = read_parquet(pred_global_path).copy()
    pred_oper = read_parquet(pred_oper_path).copy()
    pred_pure = read_parquet(pred_pure_path).copy()
    base_global = _base_from_pred(pred_global)
    ...
    view_ml2_global = _build_view_ml2(base=base_global, run_id=run_id, source="ML2_GLOBAL")
    ...
    p2g_run, p2g_latest = _write_named_and_latest(view_ml2_global, out_dir, "view_pipeline_ml2_global", run_id)
    ...
    print(
        "     rows="
        f"ml1:{len(view_ml1):,} "
        f"ml2_global:{len(view_ml2_global):,} "
