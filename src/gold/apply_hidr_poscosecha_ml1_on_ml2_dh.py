...
IN_UNIVERSE = GOLD / "pred_poscosecha_ml2_dh_grado_dia_bloque_destino.parquet"
REGISTRY_ROOT = ROOT / "models_registry" / "ml1" / "hidr_poscosecha"
OUT_PATH = GOLD / "pred_poscosecha_ml2_hidr_grado_dia_bloque_destino.parquet"
...
def main(version: str | None = None) -> None:
    ...
    need = {"fecha", "bloque_base", "variedad_canon", "grado", "destino", "dh_dias_ml1", "fecha_post_pred_ml1"}
    miss = need - set(df.columns)
    if miss:
        raise KeyError(f"Universe DH ML2 sin columnas: {sorted(miss)}")
    ...
    print(f"[OK] Wrote: {OUT_PATH} rows={len(df):,} version={ver_dir.name} clip={lo}->{hi}")
